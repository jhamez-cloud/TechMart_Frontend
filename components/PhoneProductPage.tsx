...
export default function PhoneProductPage({ product }: { product: Product }) {
  ...
  const buildSelection = (): CartProduct | null => {
    if (!selectedVariant) return null;

    return {
      productId: product.id,       // <-- include the parent product ID
  ...
  const handleAddToCart = () => {
    ...
    addToCart(selection);  // now works with new cart logic
    setOpen(true);
  };
      ...
      <section className="grid grid-cols-1 lg:grid-cols-[1.2fr_1.5fr_1fr] gap-6 bg-white p-4 lg:p-6 rounded-xl">
        ...
        <div className="space-y-4">
              ...
              {product.variants.map((v) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    <div>
                      {v.ram && <p>{v.ram} GB RAM</p>}
                      {v.storage && <p>{v.storage} Storage</p>}
                      {v.color && <p>{v.color}</p>}
                    </div>
          ...
          <ul className="flex flex-wrap gap-2">
            {[Twitter, Facebook, Instagram, Youtube, MessageCircle].map(
              (Icon, i) => (
                <li
                  key={i}
