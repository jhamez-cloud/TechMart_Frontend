...
export default function Deals() {
  ...
  useEffect(() => {
  const countdownDate = new Date('2026-03-31T12:59:59').getTime()
  ...
  const interval = setInterval(() => {
    ...
    const hrs = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)).toString()
    const mins = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60)).toString()
    const secs = Math.floor((distance % (1000 * 60)) / 1000).toString()
    ...
    <div className='w-full flex flex-col lg:flex-row gap-4 mt-8'>
      ...
      <div className='w-full lg:w-3/4'>
        ...
        <div className='w-full bg-white rounded-md p-4 md:p-8 flex flex-col lg:flex-row gap-6'>
          ...
          {/* 🔥 IMAGE SECTION */}
          <div className='w-full lg:w-1/2 flex flex-col items-center md:flex-row'>
              ...
              <div className='grid grid-cols-4 gap-2 flex-1'>
                {[
                  ...
                  <div key={i} className='bg-[#EBEDF3] rounded-md py-2 flex flex-col items-center'>
                    ...
                    <p className='text-gray-600 text-xs'>{t.label}</p>
                  </div>
                ))}
              </div>
